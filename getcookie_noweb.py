...
if __name__=='__main__':
    ...
    sign = driver.find_element(By.ID,"sign") # 找到签到标签  
    do_sign = sign.text
    while True:
        time_now = datetime.now().strftime("%H:%M:%S.%f")
        if time_now >= "00:00:00.000000" and time_now <= "00:00:01.000000":
    ...
    if do_sign == '签到':
        out_file.write('DatetimeBAT  恭喜您在 '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f ")+'和 '+time_now+' 时刻对 '+driver.title+' 进行了签到!!! \n')
    else :
        out_file.write('DatetimeBAT  恭喜您 '+datetime.now().strftime("%Y-%m-%d ")+do_sign+' 过了!!!\n')
    out_file.close() # 关闭文件释放内存
    driver.get('https://sctapi.ftqq.com/SCT130527TRj0on8GeSqRZhO76Fzdxrpf3.send?title=Hifini_web_Sign&desp='+'DatetimeBAT  恭喜您在 '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f ")+'和 '+time_now+' 时刻对 '+driver.title+' 进行了签到!!! \n')
